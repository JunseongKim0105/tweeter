...
$(document).ready(function () {
  ...
  const loadTweets = function () {
    $.ajax('/tweets', { method: 'GET' }).then(function (arrTweets) {
      // when resolved, run callback
  ...
  const createTweetElement = ({ user, content, created_at }) => {
    ...
    <article class="tweet">
      ...
      <footer>
        <div class="time-passed">${timeago.format(
          Number(`${created_at}`)
        )}</div>
        <div class='icons'>
  ...
  $('form').submit(function (event) {
    ...
    if (textStr === '') {
      $('#error-message').text(
        '⚠️ Text cannot be empty. Plz rspct our extrajudiciary rule for content. #kthxbai.⚠️ '
      );
    ...
    if (textStr === null) {
      $('#error-message').text(
        '⚠️ Text cannot be null. Plz rspct our interplanetary rule for this primitive data type. #kthxbai.⚠️ '
      );
      $('#error-message').slideDown('slow');
      return false;
    }
    if (textStr.length > 140) {
      $('#error-message').text(
        '⚠️ Too long. Plz rspct our constabulatory limit of 140 chars. #kthxbai.⚠️ '
      );
      $('#error-message').slideDown('slow');
      return false;
    }
    ...
    $.post(url, queryStr, function () {
      // Send data to server using HTTP POST request; equivalent to Ajax function
